...
function CustomTooltip({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-lg text-sm">
      ...
      <p className="text-gray-500">{d.count} avis</p>
    </div>
  )
}
...
export default function ReviewsChart({ data }: ReviewsChartProps) {
  if (data.length === 0) {
    return (
      <Card className="p-6 flex items-center justify-center h-48 text-gray-400 text-sm">
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2 mb-5">
        ...
        <div>
          <p className="text-sm font-semibold text-gray-800">Évolution de la note moyenne</p>
          <p className="text-xs text-gray-400">Note mensuelle moyenne sur la période</p>
        </div>
      </div>
