...
export default function AiInsightCard({
      ...
      <Card className="p-6">
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <div className="p-3 rounded-full bg-gray-100 dark:bg-slate-700 mb-3">
            <Brain className="w-6 h-6 text-gray-500 dark:text-slate-400" />
          </div>
          <p className="text-base font-medium text-gray-600 dark:text-slate-400">Aucun rapport AI disponible</p>
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">Le premier rapport sera généré automatiquement</p>
        </div>
      </Card>
    )
  }
  ...
  return (
    <Card className="p-6">
      ...
      <p className="text-base text-gray-700 dark:text-slate-200 leading-relaxed mb-4">{summary}</p>

      {recommendations && recommendations.length > 0 && (
        <div className="space-y-2.5">
          <div className="flex items-center gap-1.5">
            <Lightbulb className="w-3.5 h-3.5 text-gray-500 dark:text-slate-400" />
          ...
          {recommendations.slice(0, 3).map((rec, i) => (
            <div key={i} className="flex items-start gap-2.5 pl-1">
              <ArrowRight className="w-3.5 h-3.5 text-gray-400 dark:text-slate-400 mt-0.5 shrink-0" />
              <div className="flex-1 min-w-0">
                ...
                <p className="text-base text-gray-700 dark:text-slate-200">{rec.action}</p>
              </div>
            </div>
          ))}
        </div>
